...
- (void)setupView {
    ...
    self.thumbnailButton.frame = CGRectMake(20, 50, 45, 45);
    [self addSubview:self.thumbnailButton];
    
    self.captureButton = [CameraButton captureButtonWithMode:THCaptureButtonModeVideo];
    self.captureButton.frame = CGRectMake(([UIScreen mainScreen].bounds.size.width - 68.0) / 2, 35, 68.0, 68.0);
    ...
    _foregroundColor = [UIColor colorWithRed:1.0 green:0.734 blue:0.006 alpha:1.0];
    ...
    CGSize size = [@"VIDEO" sizeWithAttributes:@{NSFontAttributeName: [UIFont fontWithName:@"AvenirNextCondensed-DemiBold" size:17.0f], NSForegroundColorAttributeName: [UIColor whiteColor]}];
    self.videoStringWidth = size.width;
    
    _videoTextLayer.frame = CGRectMake(0, 0, 40, 20);
    _photoTextLayer.frame = CGRectMake(60, 0, 50, 20);
    ...
    self.labelContainerView.centerY += 8.0;
...
-(CATextLayer *)textLayerWithTitle:(NSString *)title {
    ...
    layer.string = [[NSAttributedString alloc] initWithString:title attributes:@{NSFontAttributeName: [UIFont fontWithName:@"AvenirNextCondensed-DemiBold" size:17.0f], NSForegroundColorAttributeName: [UIColor whiteColor]}];
